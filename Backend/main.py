...
def recommend_by_description(title):
    ...
    # Fetch additional details for recommended books
    recommended_books = []
    for rec_title in recommendations:
        book = books_collection.find_one({'title': rec_title}, {'title': 1, 'author': 1, 'image': 1, 'price': 1})
        if book:
            recommended_books.append(book_to_json(book))
